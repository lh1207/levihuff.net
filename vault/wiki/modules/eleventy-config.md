...
- **Development server**: passthrough assets are served directly during `eleventy --serve`, avoiding parallel-copy races in Eleventy 3.1.6 incremental builds.
- **Markdown**: `markdown-it` with `html: true`, `linkify: true`, `typographer: true`, plus `markdown-it-anchor` for accessible heading permalinks. The anchor plugin injects `header-anchor`, which Tailwind keeps through `@source inline()` in the CSS entrypoint.
- **Collections**: `posts` (all `src/blog/**/*.md`), `tagList` (deduplicated, sorted tags across posts).
- **Filters** (`src/filters.js`): `tagSlug`, `imageDimensions`, `dateReadable`, `dateIso` (throws with a `[dateIso]` prefix on invalid dates), `dateYMD`, `safeCdata`, `readingTime` (200 wpm, floor of "1 min read"), and `jsonScript` (safe inline JSON for Vue islands). The exported `imageBufferDimensions` helper is tested directly but is not registered as a template filter.
