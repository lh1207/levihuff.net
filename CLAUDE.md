...
A project-local Obsidian wiki at `vault/` documents *why* this codebase's architecture looks the way it does (ADRs, module notes, dependency/upgrade risk, build/deploy flows) - distinct from this file, which documents *what the current conventions are*. It's tracked and committed as part of this repo, not the global `~/.claude/vault`.

- `vault/CLAUDE.md` - the vault's own instructions and folder map.
- `vault/wiki/hot.md` - ~500-word recent-context cache, printed automatically at session start via a `.claude/settings.json` `SessionStart` hook.
- A `PostToolUse` hook (`.claude/hooks/vault-auto-commit.sh`) auto-commits `vault/` changes separately from site commits whenever an Edit/Write touches the vault. A `Stop` hook reminds to refresh `hot.md` when `vault/wiki/` changed during the session.
- When architecture shifts (a new module, a non-obvious tradeoff, a dependency pin), update the relevant note in `vault/wiki/` - don't let it drift stale.
