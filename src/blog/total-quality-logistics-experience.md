...
When machines were retired, they were staged for destruction rather than wiped in-house. The physical destruction and data sanitization procedures—degaussing, shredding, or other certified disposal methods outlined in processes like those covered by CompTIA A+—were handled by the appropriate teams. My role was to get machines correctly identified, tracked, and staged so that handoff happened cleanly.

Some machines arrived pre-imaged directly from Dell, which streamlined intake for new hardware—those could go straight into deployment rather than through the imaging pipeline first.
...
2. Verify BIOS settings and confirm no password is blocking network boot
3. PXE boot the machine and select the correct department image
4. Let the deployment run to completion
5. Set the BIOS password if not already applied
6. Verify the machine is functional and ready for the user

For machines arriving pre-imaged from Dell, steps 2 and 3 were already handled—the machine just needed verification before being put into use.
...
2. **Staging for destruction is its own process**: Correctly identifying and tracking machines for disposal is distinct from the disposal itself—both matter
